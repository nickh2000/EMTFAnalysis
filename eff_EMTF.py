...
if args.num_jobs:
  ...
  out_file = TFile('plots/efficiency/EMFTF_eff.root', 'recreate')
...
for event in range(evt_tree.GetEntries()):
    ...
    #################################################################################
        ...
        precision = (evt_tree.emtfTrack_q[track] * evt_tree.emtfTrack_pt[track] - evt_tree.recoMuon_charge[probe] *probe_pt) / probe_pt

        for i, bound in enumerate(pt_ranges):
            if evt_tree.emtfTrack_mode[track] != 15: continue
        ...
        if evt_tree.emtfTrack_pt[track] > 22 and probe_pt > PRB_PT and evt_tree.emtfTrack_mode[track] in modes: h_eta_phi_trg.Fill(probe_eta, probe_phi) #looser pt constraints (7GeV) and loser mode constraints to get more detail on phi plot
        ...
        if REQ_BX0 and evt_tree.emtfTrack_bx[track] != -1: continue #ensure muon is in BX0
