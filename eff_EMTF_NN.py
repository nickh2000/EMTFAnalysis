...
This script analyzes the efficiency of the NN, indicated by the use of emtfTrack_pt_dxy instead of emtfTrack_pt
...
if args.num_jobs:
  try: out_file =  TFile("/afs/cern.ch/user/n/nhurley/EMTFAnalyzer/AWBTools/macros/plots/tmp/eff_EMTF_NN%d.root" % (INDEX), 'recreate')
  except: out_file =  TFile("/afs/cern.ch/user/n/nhurley/EMTFAnalyzer/AWBTools/macros/plots/tmp/eff_EMTF_NN%d.root" % (INDEX), 'recreate')
else:
  out_file = TFile('plots/alignment/alignment_study_single_NN.root', 'recreate')
...
#This folder is a collection of EMTFNTuples, with cscSegments, re-emulated&unpacked hits, tracks enabled, ,and offline-reconstructed muons
...
for event in range(evt_tree.GetEntries()):
    ...
    for probe in range(evt_tree.recoMuon_size):
        ...
        if probe_pt > PRB_PT: 
            h_dxy.Fill(evt_tree.recoMuon_dxy[probe] * 100)
    ...
    ##############TRIG(Numerator) CUTS###################
        ...
        if probe_eta >= 2.3 and evt_tree.emtfTrack_pt_dxy[track] > TRG_PT: pos_end_mode.Fill(probe_phi, evt_tree.emtfTrack_mode[track]) #find incidence of trigger muons in erroneous high eta region


        #plot efficiency in space
        if evt_tree.emtfTrack_pt_dxy[track] > 22 and probe_pt > PRB_PT and evt_tree.emtfTrack_mode[track] in modes: h_eta_phi_trg.Fill(probe_eta, probe_phi) #looser pt constraints (7GeV) and loser mode constraints to get more detail on phi plot
        ...
        if evt_tree.emtfTrack_pt_dxy[track] < TRG_PT - .01: continue #single L1 trigger pt cut
            ###########################################
        ...
        if (probe_pt > PRB_PT):
            ...
            h_dxy_trg.Fill(evt_tree.recoMuon_dxy[probe] * 100)
            ...
            d_phi_12 = list(evt_tree.emtfTrack_pt_dxyLUT_deltaPh)[track][0]
