...
for event in range(evt_tree.GetEntries()):
    ...
    for hit_ref in range(evt_tree.emtfHit_size):
        ...
        if evt_tree.emtfHit_neighbor[hit_ref]: continue

        #Match to a CSC Segment
        iSeg = evt_tree.emtfHit_match_iSeg[hit_ref]
        if iSeg < 0: continue
    ...
    for probe in range(evt_tree.recoMuon_size):
        ...
        precision = (evt_tree.emtfTrack_q[track] * evt_tree.recoMuon_charge[probe] * evt_tree.emtfTrack_pt[track] - probe_pt) / probe_pt
